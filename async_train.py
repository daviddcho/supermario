...
def actor_fn(i, shared_model, replay_buffer, distance_log, env_name, device):
  ...
  while True:
    with torch.no_grad():
      x = tensor(np.float32(current_state)).unsqueeze(0).to(device)
...
def learner_fn(shared_model, target_model, replay_buffer, distance_log, env_name, device):
  ...
  for ep in tqdm(range(hp.N_EPISODES)):
    indices = np.random.choice(len(replay_buffer), hp.BATCH_SIZE, replace=False)
    states, actions, rewards, next_states, _ = zip(*[replay_buffer[i] for i in indices])
    ...
    with torch.no_grad():
      ...
      td_target = target_Q * hp.GAMMA + rewards
...
if __name__ == '__main__':
  ...
  shared_model = DuelingDQN(input_shape, n_actions).to(device)
